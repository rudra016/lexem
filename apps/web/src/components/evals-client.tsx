...
import {
  ...
  deleteCaseAction,
  runEvalAction,
  createEvalWithGeneratedCasesAction,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end gap-2">
        <button
          onClick={() => setGeneratingSuite(true)}
          disabled={!hasCurrentVersion || !hasKeys}
          title={
            !hasCurrentVersion
          ...
          className="h-10 px-4 border border-black bg-white text-black text-sm font-medium inline-flex items-center justify-center gap-2 transition-colors hover:bg-black hover:text-white disabled:opacity-50 disabled:hover:bg-white disabled:hover:text-black"
        >
          <Sparkles size={14} /> Generate cases
        </button>
        <button
      ...
      {generatingSuite && (
        <GenerateCasesModal
          ...
          onClose={() => setGeneratingSuite(false)}
        />
      )}
...
function GenerateCasesModal({
  ...
  const [name, setName] = useState("Auto-generated suite");
  ...
  function submit() {
    ...
    start(async () => {
      try {
        await createEvalWithGeneratedCasesAction({
          projectSlug,
          promptSlug,
          name,
          count,
  ...
  return (
    <Modal onClose={pending ? () => {} : onClose} wide>
      <h2 className="text-lg font-semibold mb-1 flex items-center gap-2">
        <Sparkles size={16} /> Generate eval cases
      </h2>
      <p className="text-sm text-neutral-500 mb-4">
        An LLM will read your current prompt
        {variableCount > 0 ? (
          <>
            {" "}
            and its <span className="font-mono">{variableCount}</span>{" "}
            variable{variableCount === 1 ? "" : "s"}
          </>
        ) : (
          " (no variables declared)"
        )}{" "}
        and produce {count} test cases scored by an LLM judge. You can edit any
        case afterwards — including switching it to regex or exact-match.
      </p>
      ...
      <div className="space-y-3">
        ...
        <div>
          <label className="text-xs text-neutral-600">
            Number of cases <span className="text-neutral-400">(5–15)</span>
          </label>
          <input
            type="number"
            min={5}
            ...
            value={count}
            onChange={(e) =>
              setCount(Math.min(15, Math.max(5, Number(e.target.value) || 5)))
            }
            className="w-full mt-1 px-3 py-2 border border-neutral-300 text-sm focus:border-black outline-none font-mono"
          />
        </div>
      ...
      <div className="flex justify-end gap-2 mt-6">
        ...
        <button
          ...
          className="h-9 px-4 bg-black text-white text-sm font-medium disabled:opacity-50 inline-flex items-center justify-center gap-2"
        >
          {pending ? <Spinner size={14} /> : <Sparkles size={14} />}
